...
Most analytics tools assume that the data already contains the answers. In practice, however, many analytical questions require context beyond the dataset.
...
You can copy and paste or download and upload the data to the system.
...
Note: The current implementation is a working prototype/demonstration intended to showcase the AI-driven analytical workflow. Most of the functionality is limited, and it’s a proof-of-concept with minimal use cases.
...
* suggesting the next analytical steps
...
This transforms analytics from simple data exploration into context discovery, as it applies the context to extend the data.
...
### More use cases and improve the codebase

I narrowed down the system to show the core on really specific use cases.
So I would consider

* Similar use cases
* Improve the mapping system for better insights.
...
A natural evolution of this system would be to integrate the Model Context Protocol (MCP).
...
This would enable a more agentic architecture in which the AI can discover and use new capabilities without requiring hard-coded integrations.